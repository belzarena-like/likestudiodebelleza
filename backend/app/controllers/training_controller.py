...
def list_training_sessions(
    ...
    items, total = training_service.list_sessions(db, query, category, is_active, limit, offset)
    return schemas.TrainingSessionSearchResponse(items=items, total=total, limit=limit, offset=offset)
...
def remove_video_from_session(session_id: int, video_id: int, db: Session = Depends(get_db)):
...
def reorder_session_videos(session_id: int, order: list[dict], db: Session = Depends(get_db)):
...
def create_video_record(payload: schemas.TrainingVideoCreate, db: Session = Depends(get_db)):
...
def update_video(video_id: int, payload: schemas.TrainingVideoCreate, db: Session = Depends(get_db)):
...
@router.get("/sessions/{session_id}/users", response_model=schemas.TrainingUserAccessSearchResponse)
def list_session_users(
    ...
    return schemas.TrainingUserAccessSearchResponse(items=items, total=total, limit=limit, offset=offset)


@router.post("/sessions/{session_id}/users", response_model=schemas.TrainingUserAccessRead)
def create_user_access(
    ...
    try:
        ...
        return training_service.create_user_access(db, session_id, payload, password_hash)
    except ValueError as e:
...
def update_user_access(
    ...
    access = training_service.update_user_access(db, access_id, is_active, password_hash, extend_days)
    if not access:
...
def academy_login(
    username: str = Form(...), password: str = Form(...), session_id: int = Form(...), db: Session = Depends(get_db)
):
...
def get_session(session_id: int, token: str = Query(...), db: Session = Depends(get_db)):
...
def get_stream_url(
    video_id: int, token: str = Query(...), db: Session = Depends(get_db), request: Request = None
):
    ...
    link = training_service.verify_video_access(db, access.training_session_id, video_id)
    if not link:
    ...
    training_service.log_video_view(db, access.id, video_id, access.training_session_id, ip_address, user_agent)
    
    # Generate signed URL
    return s3_service.generate_download_url(video.s3_key, video.s3_bucket, video.mime_type)
