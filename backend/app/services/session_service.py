...
class SessionService:
    ...
    @staticmethod
    def upsert_session(
        db: Session, payload: schemas.TreatmentSessionUpsert
    ) -> models.TreatmentSession:
    ...
    @staticmethod
    def update_session(
        db: Session, session_id: int, payload: schemas.TreatmentSessionUpdate
    ) -> models.TreatmentSession:
    ...
    def delete_session(db: Session, session_id: int) -> schemas.SessionDeleteResponse:
        ...
        return schemas.SessionDeleteResponse(
            deleted=True, message=f"Session {session_id} deleted successfully"
        )
    ...
    def search_sessions(
        ...
        items, total = crud.search_admin_sessions(
            db, query, client_id, status, limit, offset
        )
    ...
    @staticmethod
    def get_session_agenda(
        db: Session, appointment_date: date
    ) -> schemas.SessionAgendaResponse:
    ...
    @staticmethod
    def mark_attendance(db: Session, payload: schemas.SessionAttendanceUpdate) -> dict:
        ...
        if payload.attended:
            ...
            session.completed_sessions = min(
                session.completed_sessions + 1, session.planned_sessions
            )
        else:
    ...
    def get_session_history(
        ...
        items, total = crud.get_session_appointment_history(
            db, session_id, limit, offset
        )
        return schemas.SessionAppointmentHistoryResponse(
            items=items, total=total, limit=limit, offset=offset
        )

    @staticmethod
    def create_session_appointment(
        db: Session, session_id: int, payload: schemas.SessionAppointmentCreate
    ) -> models.Appointment:
