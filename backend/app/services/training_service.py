...
class TrainingService:
    ...
    def list_sessions(
        ...
        items = db.scalars(
            q.order_by(models.TrainingSession.created_at.desc())
            .limit(limit)
            .offset(offset)
        ).all()
    ...
    def create_session(
        self, db: Session, payload: schemas.TrainingSessionCreate
    ...
    def get_session_with_videos(
        self, db: Session, session_id: int
    ) -> Optional[schemas.TrainingSessionWithVideos]:
        ...
        videos = [
            sv.video for sv in sorted(session.videos, key=lambda x: x.display_order)
        ]
    ...
    def reorder_session_videos(
        self, db: Session, session_id: int, order: List[dict]
    ) -> bool:
    ...
    def create_video(
        self, db: Session, payload: schemas.TrainingVideoCreate
