...
(function () {
  ...
  if (consentId && isEditMode && window.APP_CONFIG) {
    ...
    .then(consent => {
      ...
      // Restore medical_conditions checkboxes
      if (consent.medical_conditions) {
        try {
          const mc = JSON.parse(consent.medical_conditions);
          if (mc.conditions && Array.isArray(mc.conditions)) {
            mc.conditions.forEach(condition => {
              const checkbox = document.querySelector(`input[name="conditions"][value="${condition}"]`);
              if (checkbox) checkbox.checked = true;
            });
          }
          if (mc.appointment_policy_accepted) {
            const checkbox = document.getElementById('appointment_policy');
            if (checkbox) checkbox.checked = true;
          }
          if (mc.history) document.getElementById('history').value = mc.history;
          if (mc.profession) document.getElementById('profession').value = mc.profession;
          if (mc.observations) document.getElementById('observations').value = mc.observations;
        } catch(e) {
          console.error('Error parsing medical_conditions:', e);
        }
      }
